...
Return ONLY valid JSON:
{ "riskScore": number, "note": "one sentence explaining key drivers" }`;

  const user = `Vitals: ${JSON.stringify(healthState.vitals || {})}
...
Lifestyle: sleep ${healthState.lifestyle?.sleepHours || 'unknown'} hrs/night, exercise ${healthState.lifestyle?.exercisePerWeek || 'unknown'} days/week`;
  ...
  const raw = await callClaude(system, user);
  return safeParseJSON(raw); // ✅ FIXED
}

// ── HEALTH TWIN: SCENARIO ─────────────────
async function runScenario(healthState, interventions, label) {
  const system = `You are a health scenario advisor.

Return ONLY valid JSON:
{
  "projectedRisk": number,
  "causalExplanation": "2-3 sentences explaining why"
}`;
...
Scenario: ${label}
Changes: ${JSON.stringify(interventions)}`;
