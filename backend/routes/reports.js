...
router.post('/upload', auth, upload.single('report'), async (req, res) => {
  try {
    ...
    // ✅ 5. SAFE vitals update (FIXED)
    if (!profile.healthState)
      profile.healthState = { vitals: {}, lifestyle: {}, conditions: [], medications: [] };

    (analysis.extractedValues || []).forEach(v => {
      const name = (v?.name || '').toLowerCase();

      if (name.includes('glucose') || name.includes('blood sugar'))
        profile.healthState.vitals.glucose = v?.value;

      if (name.includes('hba1c'))
        profile.healthState.vitals.hba1c = v?.value;

      if (name.includes('systolic'))
        profile.healthState.vitals.bloodPressureSystolic = v?.value;

      if (name.includes('diastolic'))
        profile.healthState.vitals.bloodPressureDiastolic = v?.value;
    });
