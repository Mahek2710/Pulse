  ...
  try {
    ...
    if (nextStep === "INTAKE_COMPLETE") {
      ...
      try {
        ...
        brief = {
          chiefComplaint:
            session.transcript.find((m) => m.role === "patient")?.content ||
            "General concern",
          aiSummary:
            "Patient reported symptoms. Clinical evaluation recommended.",
          urgencyLevel: "low",
        };
      }

      session.doctorBrief = {
        urgencyLevel: brief.urgencyLevel || "low",
        chiefComplaint: brief.chiefComplaint || "General check-in",
        keySymptoms: brief.keySymptoms || [],
        redFlags: brief.redFlags || [],
        currentMedications: brief.currentMedications || [],
...
router.get("/sessions", auth, async (req, res) => {
  try {
    const profile = await Patient.findById(req.user.patientProfileId).select(
      "intakeSessions",
    );

    const sessions = profile.intakeSessions
      .map((s) => ({
        _id: s._id,
